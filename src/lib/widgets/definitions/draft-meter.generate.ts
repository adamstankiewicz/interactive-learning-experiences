  ...
  async generate(ctx) {
    ...
    const spec = await generateStructured({
      ...
      system: [
        ...
        '',
        'WHAT IS ON SCREEN, exhaustively: the passage if you supplied one, your question, and one',
        'empty textbox. Nothing else exists. There is no list of claims, no multiple-choice options,',
        'no numbered items, no diagram, no second passage — the spec has no field for any of them,',
        'so anything your question refers to that you did not put in the passage is simply not there.',
        'A question like "read the three student claims below and judge each one" renders as a',
        'question with nothing below it, and the student cannot answer it. If a task needs material',
        'like that, put the material inside the passage itself, or ask a different question.',
        'Write the question as one thing to respond to, answerable in a few sentences of prose.',
        ...mode,
        '',
