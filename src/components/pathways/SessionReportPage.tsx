...
/**
 * The per-step evidence strip: one cell per plan step. 'unreached' also
 * covers events recorded before stepIndex was persisted — the strip never
 * guesses. Each cell carries its word in the tooltip and for screen readers.
 */
const STRIP_CELL: Record<StepOutcome, { className: string; label: string }> = {
  'first-try': { className: 'bg-success', label: 'first try' },
  attempts: { className: 'border border-warning bg-warning/25', label: 'needed attempts' },
  wrong: { className: 'bg-destructive', label: 'still wrong' },
  unreached: { className: 'bg-muted', label: 'not reached' },
};
    ...
    <span className="flex items-center gap-1">
      {strip.map((outcome, index) => (
        <span
          ...
          className={`h-3 w-4 rounded-xs ${STRIP_CELL[outcome].className}`}
        />
      ))}
    </span>
  );
